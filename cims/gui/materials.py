...
import json
from commands import materials
        ...
        if windows.materials == 0:
            ...
            self.new_btn = tk.Button(self.window,
                ...
                command = lambda:self.mc.add(
                    self.material_id_entry.get(),   #1
                    self.name_entry.get(),          #2
                self.health_entry.get(),            #3
                    self.flammability_entry.get(),  #4
                    self.reactivity_entry.get(),    #5
                    self.ppe_entry.get(),           #6
                    self.description_entry.get(),   #7
                    self.comment_text.get(),        #8
                    self.vendor_entry.get(),        #9
                    self.ink_system_entry.get(),    #10
                    self.drum_weight_entry.get(),   #11
                    self.cost_entry.get(),          #12
                    self.reorder_point_entry.get(), #13
                    self.stock_warning_entry.get(), #14
                    self.lvalue_entry.get(),        #15
                    self.avalue_entry.get(),        #16
                    self.bvalue_entry.get(),        #17
                ))
            ...
            self.back_btn = tk.Button(self.window,
                text = 'Back',
                font=tuple(self.fonts["button"]),
                command = lambda:self.mc.exit(self.window))
            ...
            # Frame
            self.voc_frame = tk.Frame(self.window, bg="black")
            self.swatch_frame = tk.Frame(self.window, bg="green")
            # Place frames
            self.voc_frame.place(x=15, y=265, width=490, height=325)
            self.swatch_frame.place(x=630, y=515, width=150, height=80)
